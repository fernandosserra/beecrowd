...
double calculateLowerArea(double matrix[SIZE][SIZE], char operation);
...
int main() {
    ...
    double result = calculateLowerArea(matrix, operation);
    printf("%.1lf\n", result);
...
// Function calculateLowerArea
double calculateLowerArea(double matrix[][SIZE], char operation) {
    double sum = 0.0;
    ...
    for (int i = 0; i < SIZE; i++) { 
        for (int j = 0; j < SIZE; j++) { 
            ...
            if (i > 6 && j > (SIZE - 1 - i) && j < i ) { // We keep using size to calculate lower area and escalate in future
                sum += matrix[i][j];
