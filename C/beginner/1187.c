...
double calculateSuperiorArea(double matrix[SIZE][SIZE], char operation);
...
int main() {
    ...
    double result = calculateSuperiorArea(matrix, operation);
    printf("%.1lf\n", result);
...
// Function calculateSuperiorArea
double calculateSuperiorArea(double matrix[][SIZE], char operation) {
    double sum = 0.0;
    ...
    for (int i = 0; i < SIZE; i++) { 
        for (int j = 0; j < SIZE; j++) { 
            ...
            if (i < 5 && j > i && j < (SIZE - 1 - i)) { // We keep using size to calculate the upper area and escalate in future
                sum += matrix[i][j];
