...
main = do
    ...
    let result = calculateLeftArea ourMatrix operation
...
-- This function calculates the sum or average of the elements in the Left Area (Problem 1189).
calculateLeftArea :: [[Double]] -> Char -> Double
calculateLeftArea ourMatrix operation =
    let
        ...
        totalSum = sum elementsOnLeftArea
        -- The number of elements in the superior area.
        count = fromIntegral (length elementsOnLeftArea) -- 'fromIntegral' to convert Int to Double
    in
        case operation of
            'S' -> totalSum
