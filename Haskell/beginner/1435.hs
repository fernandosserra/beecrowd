...
generateMatrix n = 
    ...
    [ [ calculateValue i j n | j <- [0 .. n-1] ] -- for each column in [0 .. n-1]
    | i <- [0 .. n-1] ]

-- Pure function to find positions
calculateValue :: Int -> Int -> Int -> Int
calculateValue i j n = 
    let up_distance    = i
        down_distance  = n - 1 - i
        left_distance  = j
        right_distance = n - 1 - j
        
        -- Finding minimum distance
        min_dist = min up_distance (min down_distance (min left_distance right_distance))
    in 
        min_dist + 1 -- adding 1 to the minimum distance
