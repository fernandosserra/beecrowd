...
main = do
    ...
    let result = calculateBelowSecDiagonal ourMatrix operation
...
-- This function calculates the sum or average Below the secondary Diagonal
calculateBelowSecDiagonal :: [[Double]] -> Char -> Double
calculateBelowSecDiagonal ourMatrix operation =
    let
        ...
        -- The sum of all elements Below the secondary diagonal
        totalSum = sum elementsBelowSecDiagonal
        -- The number of elements Below the secondary diagonal
        count = fromIntegral (length elementsBelowSecDiagonal) -- 'fromIntegral' to convert Int for Double
    in
        case operation of
            'S' -> totalSum
