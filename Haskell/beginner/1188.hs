...
main = do
    ...
    let result = calculateLowerArea ourMatrix operation
...
calculateLowerArea :: [[Double]] -> Char -> Double
calculateLowerArea ourMatrix operation =
    let
        ...
        elementsOnLowerArea =
            ...
            , (rowIndex > 6 && colIndex < rowIndex && colIndex > (matrixSize - 1 - rowIndex))
            ]
        
        -- The sum of all elements in the superior area.
        totalSum = sum elementsOnLowerArea
        -- The number of elements in the superior area.
        count = fromIntegral (length elementsOnLowerArea) -- 'fromIntegral' to convert Int to Double
    in
        case operation of
            'S' -> totalSum
