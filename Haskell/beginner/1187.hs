...
main = do
    ...
    let result = calculateSuperiorArea ourMatrix operation
...
-- This function calculates the sum or average of the elements in the Superior Area (Problem 1187).
calculateSuperiorArea :: [[Double]] -> Char -> Double
calculateSuperiorArea ourMatrix operation =
    let
        ...
        elementsOnSuperiorArea =
            ...
            -- The CORRECT condition for the area in Problem 1187:
            -- rowIndex < 5           -> For the upper half of the matrix
            -- colIndex > rowIndex    -> Above the main diagonal
            -- colIndex < (matrixSize - 1 - rowIndex) -> Above the secondary diagonal (for 0-indexed matrix)
            , (rowIndex < 5 && colIndex > rowIndex && colIndex < (matrixSize - 1 - rowIndex))
            ]
        
        -- The sum of all elements in the superior area.
        totalSum = sum elementsOnSuperiorArea
        -- The number of elements in the superior area.
        count = fromIntegral (length elementsOnSuperiorArea) -- 'fromIntegral' to convert Int to Double
    in
        case operation of
            'S' -> totalSum
